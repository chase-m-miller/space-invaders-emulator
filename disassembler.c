...
int Disassemble8080Op(unsigned char *codebuffer, int pc)
{
	...
	switch (*code)
	{
		...
		case 0x22: printf("SHLD   #$%02x%02x", code[2], code[1]); opbytes = 3; break; // Keep an eye on this one, note byte order
		...
		case 0x2a: printf("LHLD   #$%02x%02x", code[2], code[1]); opbytes = 3; break; // this one too
		...
		case 0xdb: printf("IN     #$%02x", code[1]); opbytes = 3; break;
